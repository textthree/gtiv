...
// add the element x onto the heap. The complexity is
...
func (t *Timer) add(td *TimerData) {
	...
	if td.index == 0 {
		...
		t.signal.Reset(d)
		if Debug {
			logger.Infof("timer: add reset delay %d ms", int64(d)/int64(itime.Millisecond))
		}
	}
	if Debug {
		logger.Infof("timer: push item key: %s, expire: %s, index: %d", td.Key, td.ExpireString(), td.index)
	}
}
...
func (t *Timer) del(td *TimerData) {
	...
	if i < 0 || i > last || t.timers[i] != td {
		...
		if Debug {
			logger.Infof("timer del i: %d, last: %d, %p", i, last, td)
		}
		return
	}
	if i != last {
	...
	if Debug {
		logger.Infof("timer: remove item key: %s, expire: %s, index: %d", td.Key, td.ExpireString(), td.index)
	}
}

// 连接 expire 秒过期，如果超过 expire 秒没有收到客户端发送的心跳则断开连接
func (t *Timer) Set(td *TimerData, expire int64) {
	t.lock.Lock()
	t.del(td)
	td.expire = itime.Now().Add(itime.Duration(expire) * itime.Second)
	t.add(td)
	t.lock.Unlock()
}
...
func (t *Timer) expire() {
	...
	for {
		...
		if fn == nil {
			...
			if Debug {
				logger.Infof("timer key: %s, expire: %s, index: %d expired, call fn", td.Key, td.ExpireString(), td.index)
			}
			fn()
		}
		t.lock.Lock()
	...
	t.signal.Reset(d)
	if Debug {
		logger.Infof("timer: expier reset delay %d ms", int64(d)/int64(itime.Millisecond))
	}
	t.lock.Unlock()
}
