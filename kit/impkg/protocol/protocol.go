...
import (
	...
	"gtiv/kit/impkg/encoding/binary"
...
func (p *Proto) WriteTo(b *bytes.Writer) {
	...
	// 跨语言通信，使用字节序保存包长度，追加到消息体前面
	// java 可使用：ByteBuffer.wrap(bytes16).order(ByteOrder.BIG_ENDIAN).getInt() 获取长度值
...
func (p *Proto) ReadTCP(rr *bufio.Reader) (err error) {
	...
	if packLen > _maxPackSize {
		logger.Cred("包太大3 ", "packLen:", packLen, "_maxPackSize:", _maxPackSize)
...
func (p *Proto) ReadWebsocket(ws *websocket.Conn) (err error) {
	...
	if packLen < 0 || packLen > _maxPackSize {
		logger.Cred("包太大2", packLen, _maxPackSize)
		return ErrProtoPackLen
	}
	if headerLen != _rawHeaderSize {
		return ErrProtoHeaderLen
