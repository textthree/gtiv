...
type Bucket struct {
	...
	routines    []chan *grpcapi.BroadcastRoomReq
	routinesNum uint64
...
func NewBucket(c *BucketConf) (b *Bucket) {
	b = new(Bucket)
	b.chs = make(map[string]*Channel, c.Channel)
	b.ipCnts = make(map[string]int32)
	b.c = c
	b.rooms = make(map[string]*Room, c.Room)
	b.routines = make([]chan *grpcapi.BroadcastRoomReq, c.RoutineAmount)
	for i := uint64(0); i < c.RoutineAmount; i++ {
		c := make(chan *grpcapi.BroadcastRoomReq, c.RoutineSize)
		b.routines[i] = c
		go b.roomproc(c)
	}
	return
...
// 每个 channel 都属于一个 room，适用于直播场景
func (b *Bucket) Put(roomId string, ch *Channel) (err error) {
	var (
		room *Room
		ok   bool
	)
...
func (b *Bucket) BroadcastRoom(arg *grpcapi.BroadcastRoomReq) {
...
// roomproc
func (b *Bucket) roomproc(c chan *grpcapi.BroadcastRoomReq) {
	for {
		arg := <-c
