...
// PeerConnection/
var pc = null;
// 房间ID, 从 url 中获取: getQueryVariable('room');
var roomid = "room-1";
var mySocketId = ""
var socket = null;
// offer 描述
...
// 状态机，初始为init
var state = 'init';
var evtColor = "background: #69c0ff; color: white; padding: 2px"
var msgColor = "background: #559897; color: white; padding: 2px"
var singnalServer = "http://localhost:8008"
...
function conn() {
  // 连接信令服务器
  socket = io(singnalServer);
  socket.on('connect', function(){
    console.info("[connect]: socket.io connected on " + singnalServer)
    // 加入房间，发送 join 消息，链接是异步的，需要延迟一会等连接成功再发送 join
    setTimeout(() => {
  ...
  socket.on('disconnect', (socket) => {
    console.log('[disconnect]: socket.io disconnect ', socket);
    if (!(state === 'leaved')) {
      // 挂断呼叫
      hangup();
      // 关闭本地媒体
  ...
  socket.on('joined', (roomid, socketId) => {
    state = 'joined' // 按钮状态机变更为 joined
    mySocketId = socketId
    console.log("%cjoined", evtColor,'加入房间 ' + roomid + " 成功，socketId: " + socketId );
  ...
  // 响应 otherjoin 消息
  socket.on('otherjoin', (fromSocketId) => {
    if(fromSocketId == mySocketId) {
      return
    }
    console.log('%cotherjoin', evtColor, '有人进入房间，socketId:', fromSocketId);
  ...
  socket.on('full', (roomid, socketId) => {
    console.log('%cfull', evtColor, '房间满员，加入失败', roomid, socketId);
    // 关闭 socket.io 连接
    socket.disconnect();
    // 挂断呼叫
    hangup();
  ...
  socket.on('leaved', (roomid, id) => {
    console.log('%cleaved', evtColor, id, " 退出房间 ", roomid);
    // 状态机变更为 leaved
    state = 'leave'
    // 关闭 socket.io 连接
    socket.disconnect();
  ...
  socket.on('bye', (room, socketId) => {
    console.log('%cbye', evtColor, "房间 ", roomid, " 断开连接 ", socketId);
  ...
  socket.on('message', (socketId, msg) => {
    ...
    console.log("%c" + data.type, msgColor, " webrtc 中转消息 ");
...
function sendMessage(roomid, data) {
  ...
  socket.emit('message', roomid, JSON.stringify(data));
}
